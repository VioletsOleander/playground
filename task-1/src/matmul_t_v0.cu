...
__global__ void MatMulFP16(const int m, const int n, const int k,
    ...
    half *sm_tileA = sm;                               // size of tileA: BLOCK_TILE_Y * BLOCK_TILE_K (elements)
    half *sm_tileB = sm + BLOCK_TILE_Y * BLOCK_TILE_K; // size of tileB: BLOCK_TILE_X * BLOCK_TILE_K (elements)
    ...
    int warpIdx_y = warpIdx / (BLOCK_TILE_X / WARP_TILE_X); // (BLOCK_TILE_X / WARP_TILE_X) = 2 warp in block x dim
...
#pragma unroll
    ...
    for (int i = 0; i < k / BLOCK_TILE_K; i++) { // exact division assumed
        ...
        // fetch gm_a[globalIdxBlk_y + (seqThreadIdx_warp + WARP_SIZE * i_warp) * 2][i * BLOCK_TILE_K + warpIdx + i_batch * N_WARP_PER_BLOCK]
...
#pragma unroll
            for (int i_warp = 0; i_warp < BLOCK_TILE_Y / WARP_SIZE / 2; i_warp++) {
                FLOAT(sm_tileA + (warpIdx + i_batch * N_WARP_PER_BLOCK) * BLOCK_TILE_Y + (seqThreadIdx_warp + WARP_SIZE * i_warp) * 2)
...
#pragma unroll
            for (int i_warp = 0; i_warp < BLOCK_TILE_X / WARP_SIZE / 2; i_warp++) {
                FLOAT(sm_tileB + (warpIdx + i_batch * N_WARP_PER_BLOCK) * BLOCK_TILE_X + (seqThreadIdx_warp + WARP_SIZE * i_warp) * 2)
...
#pragma unroll
            for (int i_frag = 0; i_frag < WARP_TILE_Y / FRAG_M; i_frag++) {
                ...
                // to frag_a[i_frag]
                wmma::load_matrix_sync(frag_a[i_frag], sm_tileA + (k_frag * FRAG_K) * BLOCK_TILE_Y + warpOffset_y + i_frag * FRAG_M,
                                       BLOCK_TILE_Y);
            }
#pragma unroll
            for (int j_frag = 0; j_frag < WARP_TILE_X / FRAG_N; j_frag++) {
                // sm_tileB[k_frag * FRAG_K (+FRAG_M)][warpOffset_x + j_frag * FRAG_N (+FRAG_N)]
                wmma::load_matrix_sync(frag_b[j_frag], sm_tileB + (k_frag * FRAG_K) * BLOCK_TILE_X + warpOffset_x + j_frag * FRAG_N,
                                       BLOCK_TILE_X);
            }
