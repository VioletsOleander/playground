...
#pragma unroll
        for (; fetchIdx < k / BLOCK_TILE_K && fetchIdx < computeIdx + N_PIPELINE_STAGE; fetchIdx++) {
            ...
            // each warp is responsible for fetching BLOCK_TILE_Y / 8 / N_WARP_PER_BLOCK = 256 / 64 = 4 data units
            // for warp:
            // gm_a[globalIdxBlk_y + warpIdx * 32 + unitIdx * 8 (+8)][fetchIdx * BLOCK_TILE_K (+32)] to
            // sm_tileA[warpIdx * 16 + unitIdx * 4 (+4)][0 (+64)]
            // for thread:
            // gm_a[globalIdxBlk_y + warpIdx * 32 + unitIdx * 8 + seqThreadIdx_warp / 4][fetchIdx * BLOCK_TILE_K + seqThreadIdx % 4 * 8 (+8)] to
            // sm_tileA[warpIdx * 16 + unitIdx * 4 + rowBase][colBase ^ rowBase * 8 (+8)]
            for (int unitIdx = 0; unitIdx < BLOCK_TILE_Y / 8 / N_WARP_PER_BLOCK; unitIdx++) {
                int col = colBase ^ rowBase;
                int row = warpIdx * 16 + unitIdx * 4 + rowBase;
                cuda::memcpy_async(FLOAT4(sm_tileA + bufferOffset + row * 64 + col * 8),
                                   FLOAT4(gm_a + (globalIdxBlk_y + warpIdx * 32 + unitIdx * 8 + seqThreadIdx_warp / 4) * k + fetchIdx * BLOCK_TILE_K + seqThreadIdx_warp % 4 * 8),
                                   4 * sizeof(float), pipeline);
            }

            // each warp is responsible for fetching BLOCK_TILE_X / 8 / N_WARP_PER_BLOCK = 128 / 64 = 2 data units
            // for wrap:
            // gm_b[fetchIdx * BLOCK_TILE_K (+32)][globalIdxBlk_x + warpIdx * 16 + unitIdx * 8 (+8)] to
            // sm_tileB[warpIdx * 8 + unitIdx * 4 (+4)][0 (+64)]
            // for thread:
            // gm_b[fetchIdx * BLOCK_TILE_K + seqThreadIdx_warp % 4 * 8 (+8)][globalIdxBlk_x + warpIdx * 16 + unitIdx * 8 + seqThreadIdx_warp / 4] to
            // sm_tileB[warpIdx * 8 + unitIdx * 4 + rowBase][colBase ^ rowBase * 8 (+8)]
            for (int unitIdx = 0; unitIdx < BLOCK_TILE_X / 8 / N_WARP_PER_BLOCK; unitIdx++) {
                int col = colBase ^ rowBase;
                int row = rowBase + warpIdx * 8 + unitIdx * 4;
                cuda::memcpy_async(FLOAT4(sm_tileB + bufferOffset + row * 64 + col * 8),
                                   FLOAT4(gm_b + (globalIdxBlk_x + warpIdx * 16 + unitIdx * 8 + seqThreadIdx_warp / 4) * n + fetchIdx * BLOCK_TILE_K + seqThreadIdx_warp % 4 * 8),
                                   4 * sizeof(float), pipeline);
            }
...
#pragma unroll
            for (int i_frag = 0; i_frag < WARP_TILE_Y / 16; i_frag++) {
                ...
                // for sm_tileA, 2 chunks are stored consecutively
...
#pragma unroll
            for (int j_frag = 0; j_frag < WARP_TILE_X / 8; j_frag += 2) {
                ...
                // for sm_tileB, one chunk is stored consecutively
