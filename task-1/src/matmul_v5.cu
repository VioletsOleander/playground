...
#pragma unroll
        for (int i_warp = 0; i_warp < 4; i_warp++) {
            sm_tileA[warpIdx * BLOCK_TILE_Y + seqThreadIdx_warp + WARP_SIZE * i_warp] =
                gm_a[(i * BLOCK_TILE_K + warpIdx) * k + globalIdxBlk_y + seqThreadIdx_warp + WARP_SIZE * i_warp];
        }
...
#pragma unroll
        for (int i_warp = 0; i_warp < 4; i_warp++) {
            sm_tileB[warpIdx * BLOCK_TILE_X + seqThreadIdx_warp + WARP_SIZE * i_warp] =
                gm_b[(i * BLOCK_TILE_K + warpIdx) * n + globalIdxBlk_x + seqThreadIdx_warp + WARP_SIZE * i_warp];
        }
