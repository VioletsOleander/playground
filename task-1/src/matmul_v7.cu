...
__global__ void MatMulFP32(const int m, const int n, const int k,
    ...
    int bufferSize_sm = blockDim.y * THREAD_TILE_Y * BLOCK_TILE_K + BLOCK_TILE_K * THREAD_TILE_X * blockDim.x;
...
#pragma unroll
        for (; fetchIdx < k / BLOCK_TILE_K && fetchIdx < computeIdx + N_PIPELINE_STAGE; fetchIdx++) {
            int bufferOffset = (fetchIdx % N_PIPELINE_STAGE) * bufferSize_sm;
...
#pragma unroll
                for (int i_warp = 0; i_warp < 4; i_warp++) {
                    cuda::memcpy_async(sm_tileA + bufferOffset + (warpIdx + 8 * i_batch) * BLOCK_TILE_Y + seqThreadIdx_warp + WARP_SIZE * i_warp,
                                       gm_a + (fetchIdx * BLOCK_TILE_K + warpIdx + 8 * i_batch) * k + globalIdxBlk_y + seqThreadIdx_warp + WARP_SIZE * i_warp,
...
#pragma unroll
                for (int i_warp = 0; i_warp < 4; i_warp++) {
                    cuda::memcpy_async(sm_tileB + bufferOffset + (warpIdx + 8 * i_batch) * BLOCK_TILE_X + seqThreadIdx_warp + WARP_SIZE * i_warp,
                                       gm_b + (fetchIdx * BLOCK_TILE_K + (warpIdx + 8 * i_batch)) * n + globalIdxBlk_x + seqThreadIdx_warp + WARP_SIZE * i_warp,
                                       sizeof(float), pipeline);
                }
            }
        ...
        int bufferOffset = (computeIdx % N_PIPELINE_STAGE) * bufferSize_sm;
...
#pragma unroll
            for (int _i = 0, _j = 0; _i < THREAD_TILE_Y && _j < THREAD_TILE_X; _i++, _j++) {
                ...
                bufferYDim_a[_i] = (sm_tileA + bufferOffset)[_k * BLOCK_TILE_Y + threadIdx.y * THREAD_TILE_Y + _i];
                // read sm_tileB[_k][threadIdx.x * THREAD_TILE_X + _j]
                bufferXDim_b[_j] = (sm_tileB + bufferOffset)[_k * BLOCK_TILE_X + threadIdx.x * THREAD_TILE_X + _j];
            }
